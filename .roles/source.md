...
lens:
  verify:
    - "Is the `mdloom:include` / `mdloom:layout` / `mdloom:table` directive syntax unambiguous? Could an author mistake a `mdloom:include` for a regular code block?"
    - "Are traceability comments (<!-- mdloom:compiled from=... -->) in the compiled output useful? Will `mdloom check` on the output produce errors pointing back to the correct md:// URI?"
    ...
    - "The `mdloom:figure id=...` comment in figure files — is this the right syntax? Could the id be inferred from the filename instead?"
    - "Watch mode output — does it clearly show which figure triggered a recompile and why?"
    - "Does `mdloom compile --check` give the same output as `mdloom compile` but without writing files? Or does it skip DaVinci validation too?"
  simplify:
...
expertise:
  ...
  domains:
    - "Source document format: mdloom: fenced block syntax"
    - "Figure files: standalone .md files with mdloom:figure comments"
    - "Compile pipeline: parse → resolve → validate → compose → write"
    - "Traceability: mdloom:compiled comments in output, md:// addresses in errors"
...
SOURCE is the role that reads a `mdloom:layout` directive in a source document and asks: "What does a first-time author think this does? And when it fails, is the error message pointing them at the figure or at the directive?"
