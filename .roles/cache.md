...
lens:
  verify:
    - "Is every cache key a deterministic hash of ALL inputs? If the mdloom version changes, does the compile cache miss? If the layout config changes (gap, align), does the compile cache miss?"
    ...
    - "Is the snapshot manifest JSON-serializable without losing type information? (The craftworks spec caught that branded types are lost in JSON.stringify — does mdloom's manifest avoid this?)"
    - "Does `mdloom cache snapshot restore` reject if compilation is in progress?"
    - "Does `mdloom cache snapshot diff` compare per-tier keys (parse + resolve + compile) or just the top-level compile key? Missing a tier comparison means the diff could miss changes."
    - "Does `mdloom compile --cache-status` show per-file, per-tier hit/miss? Is the output actionable for debugging?"
...
CACHE is the role that reads the cache key computation and asks: "Is the mdloom version in the compile cache key? What if we ship a bug fix that changes compiled output — will existing caches re-run?" And: "If a crash happens between the temp write and the rename, what does the next read see?"
