/// mdloom spec generate — analyze a resolved figure and suggest DaVinci invariants.
///
/// Works entirely offline: uses mdloom's own detection infrastructure to inspect
...
/// Format a SpecSuggestion as a TOML block ready to paste into mdloom.toml.
pub fn format_toml(spec: &SpecSuggestion) -> String {
    let mut out = String::new();

    out.push_str(
        &"# Generated by `mdloom spec generate` — review and adjust before committing\n"
            .to_string(),
    );
    out.push_str(
