...
mod tests {
    ...
    fn regex_matches_anchored_pattern() {
        ...
        assert!(evaluate_invariant_inner(&inv, "Status: DONE").is_none(),
        ...
        assert!(evaluate_invariant_inner(&inv_m, "preamble\nStatus: DONE\n").is_none(),
            "(?m) enables per-line anchors");
        // "Status: PROGRESS" — neither DONE nor WIP — fails.
        assert!(evaluate_invariant_inner(&inv, "Status: PROGRESS").is_some(),
    ...
    fn regex_invalid_pattern_reports_error() {
        ...
        assert!(result.unwrap().contains("invalid regex"), "message names the failure");
    }
