...
- Prose document directives
  - mdloom:blockquote: Indented block quote with optional attribution (markdown `>` or boxed)
  - mdloom:numbered-list (alias: mdloom:ol): Ordered (numbered) list with decimal sub-numbering
...
- Slide body directives
  ...
  - mdloom:quote: Attributed block quote (slide-only — centered with curly quotes)
...
mdloom:element kind=label field=name width=30
...
```mdloom:toc section="API Reference" max-depth=4
```
````
