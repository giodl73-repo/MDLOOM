...
use super::{expand_tokens, tokens_to_plain, consume_group};
use super::symbols::{lookup_symbol, is_font_command, is_unsupported};
use super::superscript::{to_superscript, to_subscript};
...
pub fn render_display_math(expr: &str, width: usize, align: MathAlign) -> (Vec<String>, Vec<MathDiag>) {
    let (tokens, mut diags) = tokenizer::tokenize(expr);
    let mut pos = 0;
    let rendered = render_expr(&tokens, &mut pos, &mut diags, false, 0);

    let auto_w = rendered.lines.iter()
    ...
    let lines: Vec<String> = rendered.lines.into_iter().map(|line| {
        let lw = visual_width(&line);
        if lw > out_width {
            // Clip with MATH-004
            diags.push(MathDiag {
                code: "MATH-004",
                ...
                col: 0,
                message: format!("Math expression ({} cols) exceeds declared width ({}) — clipped", lw, out_width),
            });
...
fn hconcat(parts: Vec<RenderedExpr>) -> RenderedExpr {
    ...
    let max_below = parts.iter().map(|p| p.lines.len() - 1 - p.baseline).max().unwrap_or(0);
...
fn render_display_command(
    ...
    if is_font_command(cmd) {
        let arg_toks = consume_group(tokens, pos);
        if cmd == "text" {
            let s: String = arg_toks.iter().map(|t| match t {
...
fn render_int_command(
    ...
    while *pos < tokens.len() {
        match &tokens[*pos] {
            Token::Char(c) => { integrand_text.push(*c); *pos += 1; }
            Token::Command(c) => {
                if let Some(s) = lookup_symbol(c) { integrand_text.push_str(s); }
                else { integrand_text.push_str(&format!("\\{}", c)); }
                *pos += 1;
            }
            _ => { *pos += 1; }
        }
...
fn render_sum_command(
    ...
    while *pos < tokens.len() {
        match &tokens[*pos] {
            Token::Char(c) => { body_text.push(*c); *pos += 1; }
            Token::Command(c) => {
                if let Some(s) = lookup_symbol(c) { body_text.push_str(s); }
                else { body_text.push_str(&format!("\\{}", c)); }
                *pos += 1;
            }
            _ => { *pos += 1; }
        }
...
fn render_display_env(
    ...
    while *pos < tokens.len() {
        match &tokens[*pos] {
            Token::BeginEnv(_) => { depth += 1; body.push(tokens[*pos].clone()); *pos += 1; }
            Token::EndEnv(e) => {
                depth -= 1;
                if depth == 0 {
                    if e != env {
                ...
                body.push(tokens[*pos].clone()); *pos += 1;
            }
            _ => { body.push(tokens[*pos].clone()); *pos += 1; }
        }
    }
    if depth > 0 {
        diags.push(MathDiag {
...
mod tests {
    ...
    fn display_align_center_pads() {
        ...
        assert_eq!(crate::layout::visual_width(&lines[0]), 20);
    ...
    fn display_align_left_pads() {
        ...
        assert_eq!(crate::layout::visual_width(&lines[0]), 10);
    }
    ...
    #[test]
    fn display_pmatrix() {
        let (lines, diags) = render_display_math(r"\begin{pmatrix} a & b \\ c & d \end{pmatrix}", 0, MathAlign::Left);
        assert_eq!(diags.len(), 0);
    ...
    fn display_mismatched_env_emits_math_003() {
        let (_, diags) = render_display_math(r"\begin{pmatrix} a \end{bmatrix}", 0, MathAlign::Left);
    ...
    fn display_math_004_on_overflow() {
        let (_, diags) = render_display_math(r"\alpha + \beta + \gamma + \delta + \epsilon", 5, MathAlign::Left);
    ...
    #[test]
    fn display_bmatrix() {
        let (lines, diags) = render_display_math(r"\begin{bmatrix} x & y \\ z & w \end{bmatrix}", 0, MathAlign::Left);
        assert_eq!(diags.len(), 0);
        assert!(lines[0].starts_with("⎡"));
        assert!(lines[1].starts_with("⎣"));
    }
    ...
    #[test]
    fn display_vmatrix() {
        let (lines, diags) = render_display_math(r"\begin{vmatrix} a & b \\ c & d \end{vmatrix}", 0, MathAlign::Left);
        assert_eq!(diags.len(), 0);
        assert!(lines[0].starts_with("|"));
    }
