...
pub fn render_int(
    ...
    let body_prefix = "⌡ ";
    let body_prefix_width = visual_width(body_prefix);
...
pub fn render_sum_prod(
    ...
    let op_width = visual_width(&op_str);
    let separator = " ";
    let sep_width = visual_width(separator);
    ...
    if let Some(ref upper) = upper {
        for l in &upper.lines {
            lines.push(format!("{}{}", center_in(l, limit_width), " ".repeat(sep_width + body.width)));
        }
    }
