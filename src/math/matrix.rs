...
pub fn render_matrix(kind: MatrixKind, rows: Vec<Vec<String>>) -> RenderedExpr {
    ...
    let rows: Vec<Vec<String>> = rows.into_iter().map(|mut row| {
    ...
    let col_widths: Vec<usize> = (0..n_cols).map(|c| {
        rows.iter().map(|row| visual_width(&row[c])).max().unwrap_or(0)
    }).collect();

    // Determine justification per column: if ALL cells in the column are numeric → right-align
    let col_justify: Vec<bool> = (0..n_cols).map(|c| {
    ...
    let cell_sep = "  ";
    let cell_sep_width = visual_width(cell_sep);
    let content_width: usize = col_widths.iter().sum::<usize>()
        + cell_sep_width * n_cols.saturating_sub(1);

    let rendered_rows: Vec<String> = rows.iter().map(|row| {
        let cells: Vec<String> = row.iter().enumerate().map(|(c, cell)| {
            let w = col_widths[c];
            if col_justify[c] {
...
fn delimiters(kind: MatrixKind) -> (&'static str, &'static str, &'static str, &'static str, &'static str, &'static str) {
...
mod tests {
    ...
    fn ragged_matrix_padded() {
        ...
        assert_eq!(visual_width(&r.lines[0]), visual_width(&r.lines[1]));
    }
    ...
    fn from_env_all_variants() {
        assert!(matches!(MatrixKind::from_env("pmatrix"), Some(MatrixKind::Pmatrix)));
        assert!(matches!(MatrixKind::from_env("bmatrix"), Some(MatrixKind::Bmatrix)));
        assert!(matches!(MatrixKind::from_env("matrix"),  Some(MatrixKind::Matrix)));
        assert!(matches!(MatrixKind::from_env("vmatrix"), Some(MatrixKind::Vmatrix)));
        assert!(matches!(MatrixKind::from_env("Vmatrix"), Some(MatrixKind::Vmatrix2)));
        assert!(matches!(MatrixKind::from_env("cases"),   Some(MatrixKind::Cases)));
