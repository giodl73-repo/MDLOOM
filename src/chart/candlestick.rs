...
pub fn render_candlestick_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    ...
    let ohlc: Vec<(f64, f64, f64, f64)> = data
        .0
        .iter()
        .map(|p| {
            let o = p.value;
    ...
    let xs: Vec<usize> = (0..n)
        .map(|i| (i * (plot_w - 1)) / n.saturating_sub(1).max(1))
        .collect();
    ...
    for (i, (o, h, l, c)) in ohlc.iter().enumerate() {
        ...
        let row_c = row_for(*c);
        // wick from high → low
        for r in row_h..=row_l {
            if r < height {
                canvas[r][col] = WICK;
            }
        }
        // body between open and close
        let (b_lo, b_hi) = if row_o <= row_c {
            (row_o, row_c)
        } else {
            (row_c, row_o)
        };
...
mod tests {
    ...
    fn candlestick_down_period_uses_down_body() {
        ...
        let blob = lines.join("\n");
        assert!(
            blob.contains(BODY_DOWN),
            "down period uses {} body: {}",
            BODY_DOWN,
            blob
        );
    }
