...
pub fn render_waterfall_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    ...
    let max = attrs.max.unwrap_or_else(|| running.iter().cloned().fold(f64::NEG_INFINITY, f64::max));
    let min = running.iter().cloned().chain(std::iter::once(0.0))
    ...
    for i in 0..n {
        ...
        let (lo, hi) = if prev_col <= curr_col { (prev_col, curr_col) } else { (curr_col, prev_col) };
        let glyph = if i == 0 || i == n - 1 { TOTAL }
                    else if data.0[i].value >= 0.0 { POS } else { NEG };

        let mut bar = String::with_capacity(bar_area);
        for c in 0..bar_area {
            if c >= lo && c <= hi { bar.push(glyph); } else { bar.push(' '); }
        }
...
mod tests {
    ...
    fn cfg(w: usize) -> ChartAttrs {
        ChartAttrs { kind: ChartKind::Waterfall, width: w, ..Default::default() }
    }
    fn pt(label: &str, v: f64) -> ChartPoint {
        ChartPoint { label: label.into(), value: v, extras: Vec::new() }
    }
    ...
    fn waterfall_positive_delta_uses_pos_glyph() {
        ...
        assert!(lines[1].contains(POS), "positive delta uses █: {:?}", lines[1]);
    }
    ...
    fn waterfall_negative_delta_uses_neg_glyph() {
        ...
        let lines = render_waterfall_chart(&data, &cfg(60));
        assert!(lines[1].contains(NEG), "negative delta uses ▒: {:?}", lines[1]);
    }

    #[test]
    fn waterfall_running_total_displayed() {
        let data = ChartData(vec![pt("Start", 100.0), pt("d1", 20.0), pt("d2", -30.0), pt("End", 0.0)]);
        let lines = render_waterfall_chart(&data, &cfg(60));
        // Running totals: 100, 120, 90, 90 — verify "120" appears on row 1.
        assert!(lines[1].contains("120"), "running total at d1 = 120: {:?}", lines[1]);
        assert!(lines[2].contains("90"), "running total at d2 = 90: {:?}", lines[2]);
    }
}
