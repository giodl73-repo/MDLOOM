...
pub fn render_timeline_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    ...
    let t_max = data.0.iter().map(|p| p.value).fold(f64::NEG_INFINITY, f64::max);
    let t_range = (t_max - t_min).max(1e-9);

    let plot_w = attrs.width.max(20);

    if let Some(t) = &attrs.title { out.push(center_in_width(t, plot_w)); }
    ...
    for p in &data.0 {
        ...
        if col < plot_w { marker_row[col] = MARKER; }
        tick_cols.push(col);
    }
    ...
    for &c in &tick_cols { if c < plot_w { axis[c] = TICK; } }
    ...
    for (i, p) in data.0.iter().enumerate() {
        ...
        let start = center.saturating_sub(lw / 2)
            .min(plot_w.saturating_sub(lw));
        if start >= last_end_a {
            for (j, ch) in p.label.chars().enumerate() {
                if start + j < plot_w { row_a[start + j] = ch; }
            }
            last_end_a = start + lw + 1;
        } else if start >= last_end_b {
            used_b = true;
            for (j, ch) in p.label.chars().enumerate() {
                if start + j < plot_w { row_b[start + j] = ch; }
            }
...
mod tests {
    ...
    fn cfg(w: usize) -> ChartAttrs {
        ChartAttrs { kind: ChartKind::Timeline, width: w, ..Default::default() }
    }
    fn ev(name: &str, t: f64) -> ChartPoint {
        ChartPoint { label: name.into(), value: t, extras: Vec::new() }
    }
    ...
    fn timeline_renders_markers_for_each_event() {
        ...
        let total_markers: usize = lines.iter().map(|l| l.chars().filter(|&c| c == MARKER).count()).sum();
    ...
    fn timeline_baseline_present() {
        ...
        assert!(lines.iter().any(|l| l.contains(HORIZ)), "baseline: {:?}", lines);
    }
    ...
    fn timeline_first_marker_left_of_last() {
        ...
        let lines = render_timeline_chart(&data, &cfg(40));
        let marker_row = lines.iter().find(|l| l.chars().filter(|&c| c == MARKER).count() >= 2).unwrap();
        let positions: Vec<usize> = marker_row.chars().enumerate()
            .filter(|(_, c)| *c == MARKER)
            .map(|(i, _)| i)
            .collect();
        assert!(positions[0] < positions[1], "first event left of last: {:?}", positions);
    }
}
