...
const SERIES_GLYPHS: &[char] = &['\u{2588}', '\u{2593}', '\u{2592}', '\u{2591}', '#', '*', '+'];

pub fn render_stacked_bar_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    let mut out = Vec::new();

    let label_w = data.0.iter().map(|p| p.label.chars().count()).max().unwrap_or(0);

    let totals: Vec<f64> = data.0.iter()
        .map(|p| p.value + p.extras.iter().sum::<f64>())
        .collect();
    let max_total = attrs.max.unwrap_or_else(|| totals.iter().cloned().fold(f64::NEG_INFINITY, f64::max).max(0.0));

    let total_strs: Vec<String> = totals.iter().map(|t| format_value(*t)).collect();
    let value_w = total_strs.iter().map(|s| s.chars().count()).max().unwrap_or(0);
    ...
    for (i, point) in data.0.iter().enumerate() {
        ...
        for (s_idx, sv) in series_values.iter().enumerate() {
            ...
            for _ in 0..segment { bar.push(glyph); }
            filled += segment;
        }
...
mod tests {
    ...
    fn cfg(w: usize) -> ChartAttrs {
        ChartAttrs { kind: ChartKind::StackedBar, width: w, ..Default::default() }
    }

    #[test]
    fn stacked_segments_use_distinct_glyphs() {
        let data = ChartData(vec![
            ChartPoint { label: "Q1".into(), value: 50.0, extras: vec![25.0, 10.0] },
        ]);
        let lines = render_stacked_bar_chart(&data, &cfg(60));
        let blob = lines.join("\n");
        // First three series glyphs should appear in the bar.
        assert!(blob.contains('\u{2588}'), "series 1 glyph present: {:?}", blob);
        assert!(blob.contains('\u{2593}'), "series 2 glyph present: {:?}", blob);
        assert!(blob.contains('\u{2592}'), "series 3 glyph present: {:?}", blob);
    }

    #[test]
    fn stacked_total_value_displayed() {
        let data = ChartData(vec![
            ChartPoint { label: "Q1".into(), value: 30.0, extras: vec![20.0] },
        ]);
        let lines = render_stacked_bar_chart(&data, &cfg(60));
        // Total = 50; should appear at the row's right side.
        assert!(lines[0].contains("50"), "total value rendered: {:?}", lines[0]);
    }

    #[test]
    fn stacked_segment_widths_sum_to_total_bar() {
        let data = ChartData(vec![
            ChartPoint { label: "Q1".into(), value: 80.0, extras: vec![10.0, 10.0] },
        ]);
        ...
        let glyphs_total: usize = lines[0].chars()
            .filter(|c| SERIES_GLYPHS.contains(c)).count();
        ...
        assert!(glyphs_total >= 40 && glyphs_total <= 55,
            "expected ~bar_area filled, got {}: {:?}", glyphs_total, lines[0]);
    }
}
