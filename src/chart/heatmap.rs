...
pub fn render_heatmap_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    ...
    for p in &data.0 {
        ...
        if !row_order.contains(&row) { row_order.push(row.clone()); }
        if !col_order.contains(&col) { col_order.push(col.clone()); }
        cells.insert((row, col), p.value);
    }
    if row_order.is_empty() || col_order.is_empty() { return out; }

    let max = attrs.max.unwrap_or_else(|| cells.values().cloned().fold(f64::NEG_INFINITY, f64::max).max(0.0));
    let min = cells.values().cloned().fold(f64::INFINITY, f64::min).min(0.0);
    let range = (max - min).max(1e-9);

    let row_label_w = row_order.iter().map(|s| s.chars().count()).max().unwrap_or(0);
    // Each col header gets at least its own label width or 3, whichever bigger.
    let cell_w = col_order.iter().map(|s| s.chars().count().max(3)).max().unwrap_or(3);

    if let Some(t) = &attrs.title { out.push(center_in_width(t, attrs.width)); }

    // Column header row.
    let mut header = String::new();
    for _ in 0..row_label_w + 1 { header.push(' '); }
    for c in &col_order {
    ...
    for r in &row_order {
        ...
        for c in &col_order {
            ...
            for _ in 0..cell_w { row.push(glyph); }
            row.push(' ');
        }
...
mod tests {
    ...
    fn cfg(w: usize) -> ChartAttrs {
        ChartAttrs { kind: ChartKind::Heatmap, width: w, ..Default::default() }
    }
    ...
    fn heatmap_uses_full_block_for_max() {
        ...
        let blob = lines.join("\n");
        assert!(blob.contains('\u{2588}'), "max cell uses full block: {}", blob);
    }
