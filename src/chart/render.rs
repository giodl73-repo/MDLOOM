...
use super::{bar, line, area, stacked_bar, waterfall, scatter, heatmap, candlestick, gantt, timeline};
...
pub fn parse_inline_body(body: &str) -> Result<ChartData, (usize, String)> {
    ...
    for (i, raw) in body.lines().enumerate() {
        ...
        let nums: Result<Vec<f64>, _> = val_str.split(',')
            .map(|s| s.trim().parse::<f64>())
            .collect();
        let nums = nums.map_err(|_| (i, format!("invalid number(s) {:?} for label {:?}", val_str, label)))?;
        if nums.is_empty() {
            return Err((i, format!("no numeric value(s) for label {:?}", label)));
        }
        let value = nums[0];
        let extras = nums[1..].to_vec();
        points.push(ChartPoint { label, value, extras });
    }
    Ok(ChartData(points))
}
...
mod tests {
    ...
    fn chart_kind_parse() {
        ...
        assert_eq!(ChartKind::parse("candlestick"), Some(ChartKind::Candlestick));
