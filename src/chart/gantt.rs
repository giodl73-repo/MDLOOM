...
pub fn render_gantt_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    ...
    let ends: Vec<f64> = data.0.iter().map(|p| p.extras.first().copied().unwrap_or(p.value)).collect();
...
mod tests {
    ...
    fn cfg(w: usize) -> ChartAttrs {
        ChartAttrs { kind: ChartKind::Gantt, width: w, ..Default::default() }
    }
    fn task(name: &str, start: f64, end: f64, status: f64) -> ChartPoint {
        ChartPoint { label: name.into(), value: start, extras: vec![end, status] }
    }
    ...
    fn gantt_renders_one_row_per_task() {
        ...
        for name in ["Spec", "Build", "Test"] {
            assert!(lines.iter().any(|l| l.contains(name)), "{} present: {:?}", name, lines);
        }
    }
    ...
    fn gantt_status_glyphs_distinct() {
        ...
        assert!(lines.iter().any(|l| l.contains("Done") && l.contains(STATUS_GLYPHS[0])));
        assert!(lines.iter().any(|l| l.contains("WIP")  && l.contains(STATUS_GLYPHS[1])));
        assert!(lines.iter().any(|l| l.contains("Plan") && l.contains(STATUS_GLYPHS[2])));
    }
    ...
    fn gantt_bar_position_reflects_time() {
        ...
        assert!(early_pos < late_pos, "early task left of late task: {} vs {}", early_pos, late_pos);
    }
}
