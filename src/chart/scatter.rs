...
pub fn render_scatter_chart(data: &ChartData, attrs: &ChartAttrs) -> Vec<String> {
    ...
    for (x, y) in &pts {
        ...
        if col < plot_w && row < height { canvas[row][col] = MARKER; }
    }
...
mod tests {
    ...
    fn cfg(w: usize, h: usize) -> ChartAttrs {
        ChartAttrs { kind: ChartKind::Scatter, width: w, height: h, ..Default::default() }
    }

    #[test]
    fn scatter_renders_markers() {
        let data = ChartData(vec![
            ChartPoint { label: "p1".into(), value: 1.0, extras: vec![1.0] },
            ChartPoint { label: "p2".into(), value: 5.0, extras: vec![5.0] },
            ChartPoint { label: "p3".into(), value: 10.0, extras: vec![2.0] },
        ]);
        let lines = render_scatter_chart(&data, &cfg(40, 8));
        let total: usize = lines.iter().map(|l| l.chars().filter(|&c| c == MARKER).count()).sum();
    ...
    fn scatter_baseline_present() {
        let data = ChartData(vec![
            ChartPoint { label: "p".into(), value: 1.0, extras: vec![1.0] },
            ChartPoint { label: "q".into(), value: 2.0, extras: vec![2.0] },
        ]);
        let lines = render_scatter_chart(&data, &cfg(40, 6));
        assert!(lines.iter().any(|l| l.contains(CORNER)), "└ baseline expected:\n{:?}", lines);
    }

    #[test]
    fn scatter_axis_labels_present() {
        let data = ChartData(vec![
            ChartPoint { label: "p".into(), value: 0.0, extras: vec![0.0] },
            ChartPoint { label: "q".into(), value: 10.0, extras: vec![100.0] },
        ]);
        let lines = render_scatter_chart(&data, &cfg(40, 6));
