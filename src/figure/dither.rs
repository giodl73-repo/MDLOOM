    ...
    (0..out_h).map(|row| {
        (0..out_w).map(|col| {
            ...
            let q = |dx: u32, dy: u32| -> u8 {
                let x = px + dx;
                let y = py + dy;
                if x < ctx.width && y < ctx.height && luma(ctx, x, y) >= threshold { 1 } else { 0 }
            };
...
mod tests {
    ...
    fn test_quarter_block_output_dimensions() {
        ...
        let opts = ImportOptions { dither: DitherMode::QuarterBlock, threshold: 128, ..Default::default() };
        let ctx = DitherContext::new(&img, &opts);
    ...
    fn test_quarter_block_white_is_full() {
        ...
        let rows = dither_quarter_block(&ctx);
        for row in &rows {
            assert!(row.chars().all(|c| c == '█'), "all white → full block: {:?}", row);
        }
    }
    ...
    fn test_quarter_block_black_is_space() {
        ...
        let rows = dither_quarter_block(&ctx);
        for row in &rows {
            assert!(row.chars().all(|c| c == ' '), "all black → space: {:?}", row);
        }
    }
    ...
    fn test_quarter_block_each_quadrant_distinct() {
        ...
        for &(pos, expected, name) in &[
            ...
            for y in 0..2 { for x in 0..2 { img.put_pixel(x, y, Luma([0u8])); } }
