...
fn main() -> Result<()> {
    ...
    let (paths, da_vinci) = match &cli.command {
        Some(Command::Check { paths, da_vinci, by_code, deduplicate: dedup }) => {
    ...
    cmd_check(paths, &cli, da_vinci, show_by_code, deduplicate)
}
...
fn cmd_check(paths: Vec<PathBuf>, cli: &Cli, da_vinci: bool, show_by_code: bool, deduplicate: bool) -> Result<()> {
