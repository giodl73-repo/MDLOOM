<!-- mdloom:compiled from="mdloom:slides" count=9 -->
```slides
SLIDE 1 ─────────────────────────────────────────────────────────────────────── 1/9
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                  Calculus II                                   
                             Integration Techniques                             
                                                                                
                                  Prof. Rivera                                  
                                  Spring 2026                                   
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 2 ─────────────────────────────────────────────────────────────────────── 2/9
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                         ── The Fundamental Theorem ──                          
                                                                                
                   Connecting differentiation and integration                   
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 3 ─────────────────────────────────────────────────────────────────────── 3/9
FTC Part 1                                                                      
                                                                                
                                                                                
────────────────────────────────────────────────────────────────────────────────
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 4 ─────────────────────────────────────────────────────────────────────── 4/9
                                                                                
                                                                                
                                                                                
────────────────────────────────────────────────────────────────────────────────
If f is continuous on  and F(x) = ∫ₐˣ f(t)  dt, then:                           
                                                                                
F′(x) = f(x)                                                                    
                                                                                
That is, F is an antiderivative of f. In other words, differentiation           
and integration are inverse operations.                                         
                                                                                
★ The derivative of the area function equals the original function.             
  ```                                                                           
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 5 ─────────────────────────────────────────────────────────────────────── 5/9
Integration by Parts                                                            
                                                                                
                                                                                
────────────────────────────────────────────────────────────────────────────────
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 6 ─────────────────────────────────────────────────────────────────────── 6/9
                                                                                
                                                                                
                                                                                
────────────────────────────────────────────────────────────────────────────────
For differentiable functions u and v:                                           
                                                                                
∫ u  dv = uv - ∫ v  du                                                          
                                                                                
────────────────────────────────────────────────────────────────────────────────
                                                                                
● Choose $u$ = function that simplifies when differentiated                     
● Choose $dv$ = function that is easy to integrate                              
● LIATE rule: Logarithmic → Inverse trig → Algebraic → Trig → Exponential       
● ```                                                                           
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 7 ─────────────────────────────────────────────────────────────────────── 7/9
Example — $\int x e^x dx$                                                       
                                                                                
                                                                                
────────────────────────────────────────────────────────────────────────────────
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 8 ─────────────────────────────────────────────────────────────────────── 8/9
                                                                                
                                                                                
                                                                                
────────────────────────────────────────────────────────────────────────────────
Let u = x and dv = eˣ  dx.                                                      
                                                                                
Then du = dx and v = eˣ.                                                        
                                                                                
By parts: ∫ x eˣ  dx = x eˣ - ∫ eˣ  dx = x eˣ - eˣ + C                          
                                                                                
ℹ Check by differentiation: $\frac{d}{dx}(xe^x - e^x) = e^x + xe^x - e^x = xe^  
  ```                                                                           
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
SLIDE 9 ─────────────────────────────────────────────────────────────────────── 9/9
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                    Practice                                    
               Try: $\int \ln x\, dx$ using integration by parts                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
                                                                                
```
<!-- /mdloom:compiled -->
