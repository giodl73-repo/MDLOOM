...
Filter to status=better OR best — mdloom's filter is AND-only across repeated
terms, so this scenario uses a single `!=` filter that excludes only the
baseline. The remaining three rows render as-is.

<!-- mdloom:compiled from="md://src/user-scenarios/data/models.md#:table:0?filter=status!=baseline" -->
```
