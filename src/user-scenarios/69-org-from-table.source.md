...
`model` becomes the node name. Mdloom synthesizes parent nodes for any
status that isn't itself listed as a row.

```mdloom:tree kind=org source=md://src/user-scenarios/data/models.md#:table:0 name=model parent=status
```
