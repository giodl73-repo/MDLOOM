...
```mdloom:row source=md://src/user-scenarios/data/models.md#:table:0 width=52
  mdloom:element kind=label field=model width=20
  mdloom:element kind=value field=delta width=8
  mdloom:element kind=sparkline field=val_loss width=22 no-chrome
```

The `accuracy` column from the table isn't used here — its `89.1%` cells
include a `%` and don't parse as f64. Use it via `mdloom:element kind=label
