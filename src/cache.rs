...
pub fn snapshot_save(root: &Path, name: &str) -> std::io::Result<SnapshotManifest> {
    ...
    let mut tiers: std::collections::HashMap<String, TieredCacheKeys> = std::collections::HashMap::new();
    ...
    if let Ok(entries) = std::fs::read_dir(tmp.join("compile")) {
        for entry in entries.flatten() {
            ...
            let Ok(ce) = serde_json::from_str::<CompileCacheEntry>(&text) else { continue; };
            let compile_key_str = p.file_stem().and_then(|s| s.to_str()).unwrap_or_default().to_string();
            let path_index = load_path_index(root);
            let parse_key = path_index.get(&ce.source_path)
                .map(|e| e.parse_key.clone())
                .unwrap_or_default();
            let resolve_keys = ce.resolved_uris.clone();
            files.push(ce.source_path.clone());
            tiers.insert(ce.source_path.clone(), TieredCacheKeys {
                parse: parse_key,
    ...
    if let Some(parent) = snap_root.parent() { std::fs::create_dir_all(parent)?; }
...
pub fn snapshot_list(root: &Path) -> Vec<SnapshotManifest> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(m) = load_snapshot_manifest(&p) { out.push(m); }
    }
...
pub fn snapshot_diff(root: &Path, name_a: &str, name_b: &str) -> Result<SnapshotDiff, SnapshotError> {
    ...
    let mut diff = SnapshotDiff {
        only_in_a: Vec::new(), only_in_b: Vec::new(),
        changed: Vec::new(), identical: Vec::new(),
    };
    for f in &a.files {
    ...
    for f in &b.files {
        if !a.tiers.contains_key(f) { diff.only_in_b.push(f.clone()); }
    }
    diff.only_in_a.sort();
    diff.only_in_b.sort();
    diff.changed.sort();
...
pub fn snapshot_deploy(root: &Path, name: &str, target_dir: &Path) -> Result<usize, SnapshotError> {
    ...
    let Ok(entries) = std::fs::read_dir(&compile_dir_in_snap) else { return Ok(0); };
    for entry in entries.flatten() {
        let p = entry.path();
        if p.extension().and_then(|e| e.to_str()) != Some("json") { continue; }
        let Ok(text) = std::fs::read_to_string(&p) else { continue; };
...
fn verify_integrity(manifest: &SnapshotManifest) -> Result<(), SnapshotError> {
    ...
    if stored != computed { Err(SnapshotError::IntegrityMismatch) } else { Ok(()) }
}

fn tiered_signature(t: &TieredCacheKeys) -> String {
    format!("{}|{}|{}", t.parse, t.resolve.join(","), t.compile.as_deref().unwrap_or(""))
}
...
mod tests {
    ...
    fn seed_compile_entry(root: &Path, key: &str, source_path: &str, output_path: &str, text: &str) {
        let _ = std::fs::create_dir_all(compile_dir(root));
        let entry = CompileCacheEntry {
    ...
    fn snapshot_restore_returns_manifest() {
        ...
        assert!(load_compile_cache(root, "k1").is_some(), "restored cache entry should reload");
    }
    ...
    fn snapshot_restore_rejects_corrupted_manifest() {
        ...
        assert!(matches!(result, Err(SnapshotError::IntegrityMismatch)),
            "tampered snapshot must reject restore: {:?}", result);
    }
    ...
    fn snapshot_list_orders_newest_first() {
        ...
        assert!(list.len() >= 2);
        assert_eq!(list[0].name, "new", "newest first: {:?}", list.iter().map(|m| &m.name).collect::<Vec<_>>());
    }
    ...
    fn snapshot_diff_reports_changed_files() {
        ...
        assert!(diff.changed.iter().any(|f| f == "shared.source.md"),
            "shared file with different keys should appear as changed: {:?}", diff);
    }
    ...
    fn snapshot_prune_keeps_n_most_recent() {
        ...
        assert!(remaining.iter().any(|m| m.name == "d"), "newest snapshot kept");
    }
    ...
    fn snapshot_deploy_writes_compiled_text() {
        ...
        assert!(written.contains("compiled body"), "deployed file content: {:?}", written);
    }
