...
- Preserved the existing pin behavior: resolve the supplied `md://` URI, append
  ...
  guidance.
- Added a CLI regression that verifies `mdloom pin` appends the expected
  DaVinci entry to `mdloom.toml`.
...
Larger commands remain in `main.rs`; `mdloom check` is the next natural
