...
## Practical Implications for glint
...
character widths or whitespace — it's purely visual (color). glint may use info
strings to enable language-specific validation in future.

### Rendering Parity with glint

glint uses the `unicode-width` crate (Unicode East Asian Width standard) for
