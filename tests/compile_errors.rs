...
///
/// Verifies that mdloom surfaces actionable errors — not silent empty output —
...
fn has_error(violations: &[mdloom_lib::compile::CompileViolation]) -> bool {
    violations
        .iter()
        .any(|v| v.severity == ViolationSeverity::Error)
}

fn error_codes(violations: &[mdloom_lib::compile::CompileViolation]) -> Vec<&str> {
    violations
        .iter()
...
// ─────────────────────────────────────────────────────────
// mdloom:tree — bad source references
// ─────────────────────────────────────────────────────────

#[test]
fn tree_missing_md_uri_emits_error() {
    let dir = tempfile::tempdir().unwrap();
    let src = "# Test\n\n```mdloom:tree kind=taxonomy source=md://does-not-exist.md\n```\n";
...
fn tree_wrong_column_names_emits_error() {
    ...
    .unwrap();
    let src = "# Test\n\n```mdloom:tree kind=org source=md://data.md name=nonexistent parent=also_missing\n```\n";
    let (out, violations) = compile_source(src, "test.source.md", dir.path());
    assert!(
        has_error(&violations),
...
fn tree_empty_output_produces_error_not_empty_fence() {
    ...
    std::fs::write(dir.path().join("data.md"),
        "# Features\n\n| name | category |\n|------|----------|\n| Feature A | cat1 |\n| Feature B | cat1 |\n").unwrap();
    let src = "# Test\n\n```mdloom:tree kind=taxonomy source=md://data.md name=name parent=category\n```\n";
...
// ─────────────────────────────────────────────────────────
// mdloom:element — bad source references
// ─────────────────────────────────────────────────────────
...
fn element_missing_source_uri_emits_error() {
    let dir = tempfile::tempdir().unwrap();
    let src =
        "# Test\n\n```mdloom:element kind=value field=score width=8\nmd://missing-file.md\n```\n";
...
fn element_missing_field_column_emits_error() {
    ...
    .unwrap();
    let src =
        "# Test\n\n```mdloom:element kind=value field=nonexistent_col width=8\nmd://data.md\n```\n";
    let (_, violations) = compile_source(src, "test.source.md", dir.path());
    assert!(
        has_error(&violations),
...
// ─────────────────────────────────────────────────────────
// mdloom:row — bad source references
// ─────────────────────────────────────────────────────────

#[test]
fn row_missing_source_uri_emits_error() {
    let dir = tempfile::tempdir().unwrap();
    let src = "# Test\n\n```mdloom:row source=md://missing.md foreach=row separator=\" | \"\nmdloom:element kind=label field=name width=10\n```\n";
    let (_, violations) = compile_source(src, "test.source.md", dir.path());
    assert!(
        has_error(&violations),
...
fn row_empty_table_emits_warning() {
    ...
    .unwrap();
    let src = "# Test\n\n```mdloom:row source=md://data.md foreach=row separator=\" | \"\nmdloom:element kind=label field=name width=10\n```\n";
    let (_, violations) = compile_source(src, "test.source.md", dir.path());
...
fn compile_error_does_not_write_broken_output() {
    // When a directive fails with an error, mdloom should NOT write partial output.
    ...
    assert!(
        !result.written,
        "mdloom should NOT write output when errors occur"
    );
