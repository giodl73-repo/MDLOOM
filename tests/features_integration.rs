...
fn compile_with_chart_using_uri(dir: &Path, uri: &str) -> (String, Vec<proof_lib::compile::CompileViolation>, usize) {
...
fn query_select_projects_columns() {
    ...
    let (out, violations, count) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?select=name,goals");
...
fn query_select_unknown_column_errors() {
    let dir = tempfile::tempdir().unwrap();
    write_table_fixture(dir.path());
    let (_, violations, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?select=name,bogus");
    assert!(violations.iter().any(|v| v.message.contains("?select") && v.message.contains("bogus")),
...
fn query_filter_eq_drops_non_matching_rows() {
    ...
    let (out, violations, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?filter=pos=F");
    let errs: Vec<_> = violations.iter()
        .filter(|v| matches!(v.severity, ViolationSeverity::Error)).collect();
    assert!(errs.is_empty(), "no errors: {:?}",
        errs.iter().map(|v| (v.code, &v.message)).collect::<Vec<_>>());
...
fn query_filter_gt_numeric() {
    ...
    let (out, _, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?filter=goals>50");
...
fn query_top_takes_first_n() {
    ...
    let (out, _, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?top=2");
    assert!(out.contains("McDavid"));
    assert!(out.contains("Draisaitl"));
    assert!(!out.contains("Bouchard"), "row 3 dropped by top=2:\n{}", out);
}
...
fn query_skip_drops_first_n() {
    ...
    let (out, _, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?skip=4");
...
fn query_skip_then_top() {
    ...
    let (out, _, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?skip=2&top=2");
    assert!(out.contains("Bouchard"));
    assert!(out.contains("Skinner"));
    assert!(!out.contains("McDavid"));
...
fn query_count_returns_single_cell() {
    ...
    assert!(content.contains("count"), "synthetic count column present:\n{}", content);
    assert!(content.contains("6"), "row count value present:\n{}", content);
}
...
fn query_combined_filter_top() {
    ...
    let (out, _, _) = compile_with_chart_using_uri(
        dir.path(), "md://data.md#:table:0?filter=pos=F&top=2");
    assert!(out.contains("McDavid"));
    assert!(out.contains("Draisaitl"));
    assert!(!out.contains("Gretzky"), "top=2 cuts before Gretzky:\n{}", out);
}
