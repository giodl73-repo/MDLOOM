...
///
/// Full cycle: copy fixtures → glint check --format rich → verify rich.json →
///             apply plan.json → glint check → verify zero errors → cleanup.
///
...
fn e2e_stage1_rich_output_contains_errors_with_context() {
    ...
    let out = run_glint_in(ws.path(), &[
        "check", "--format", "rich", "--no-fail",
        "-o", rich_path.to_str().unwrap(),
        ".",
    ]);
    assert!(out.status.success(), "check --format rich failed:\n{}", String::from_utf8_lossy(&out.stderr));

    // rich.json must exist and be valid JSON
    let raw = std::fs::read_to_string(&rich_path)
        .expect("rich.json not written");
    let diags: serde_json::Value = serde_json::from_str(&raw)
        .expect("rich.json is not valid JSON");
    ...
    let codes: Vec<&str> = arr.iter()
        ...
        .collect();
    assert!(codes.contains(&"ascii_box_width"), "must detect ascii_box_width in rich output");
    assert!(codes.contains(&"ascii_box_col"),   "must detect ascii_box_col in rich output");

    // Every ascii_box_* diagnostic must have a rich context block
    for diag in arr.iter().filter(|d| {
        d["code"].as_str().map_or(false, |c| c.starts_with("ascii_box"))
    }) {
        let rich = &diag["rich"];
        assert!(!rich.is_null(),
            "ascii_box diagnostic missing rich context: {}", diag["message"]);
        assert!(rich["box_opens_at"].is_number(),
            "rich.box_opens_at must be a number");
        assert!(rich["border_line"].is_string(),
            "rich.border_line must be a string");
        assert!(rich["expected_cols"].is_array(),
            "rich.expected_cols must be an array");
        assert!(rich["actual_cols"].is_array(),
            "rich.actual_cols must be an array");
        assert!(!rich["lines"].is_null(),
            "rich.lines must be present");
        ...
        assert!(rich["lines"][&line_key].is_string(),
            "context.lines must contain the failing line ({})", span_line);
    }

    // clean.md must produce zero diagnostics
    let clean_errors: Vec<_> = arr.iter()
        .filter(|d| d["file"].as_str().map_or(false, |f| f.contains("clean")))
        .collect();
    assert!(clean_errors.is_empty(),
        "clean.md should have zero diagnostics, got: {:?}", clean_errors);
}
...
fn e2e_stage3_fix_plan_resolves_all_errors() {
    ...
    let before = run_glint_in(ws.path(), &["check", "--no-fail", "."]);
    let before_stderr = String::from_utf8_lossy(&before.stderr);
    assert!(!before.status.success() || before_stderr.contains("error"),
    ...
    let dry = run_glint_in(ws.path(), &[
        "fix", "--plan", plan_path.to_str().unwrap(),
        "--dry-run", "--no-verify",
    ]);
    ...
    assert!(col_after.contains("|  bad  |  bad  |"),
    ...
    let after = run_glint_in(ws.path(), &["check", "."]);
...
fn e2e_full_pipeline_check_rich_fix_verify() {
    ...
    let rich_path = ws.path().join("rich.json");
    let s1 = run_glint_in(ws.path(), &[
        "check", "--format", "rich", "--no-fail",
        "-o", rich_path.to_str().unwrap(),
        ".",
    ]);
    assert!(s1.status.success(), "Stage 1 failed: {}", String::from_utf8_lossy(&s1.stderr));
    ...
    let errors: Vec<_> = diags.as_array().unwrap().iter()
        .filter(|d| d["severity"] == "error")
        .collect();
    assert!(!errors.is_empty(), "Stage 1: must detect at least one error");
    assert!(
    ...
    // Stage 3: apply plan
    let s3 = run_glint_in(ws.path(), &[
        "fix", "--plan", plan_path.to_str().unwrap(), "--no-verify",
    ]);
    assert!(s3.status.success(), "Stage 3 failed: {}", String::from_utf8_lossy(&s3.stderr));

    // Stage 4: verify
    let s4 = run_glint_in(ws.path(), &["check", "."]);
    assert!(
        s4.status.success(),
    ...
    assert_eq!(clean_original, clean_after, "clean.md must be byte-identical after pipeline");
