...
| Config struct | `src/config.rs` — `#[derive(Debug, Deserialize, Clone)]`, `Default` impl, wired into `GlintConfig` |
...
Add `ascii_chart: AsciiChartConfig` to `GlintConfig`. Merge: child wins (same as `ascii_barchart`).
