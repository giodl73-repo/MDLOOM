# mdloom User Scenarios

25 real use cases, each with a person, a goal, the mdloom commands to accomplish it,
and the result. Every scenario is runnable against the current mdloom binary.
...
```bash
mdloom spec-generate "md://docs/arch.md:figure:system-overview" --id system-overview
# paste output into mdloom.toml
...
```bash
mdloom check . --errors-only              # see the 47 errors
mdloom fix . --min-confidence high --dry-run  # preview
mdloom fix . --min-confidence high        # apply
mdloom check .                            # verify clean
```
...
## US-07 — TUI developer embedding mdloom-canvas

**Who**: A Rust developer building a terminal dashboard app.
**Goal**: Use mdloom-canvas as a layout primitive — paste regions at exact positions.
...
**Goal**: `mdloom check` runs in GitHub Actions and fails the PR if docs degrade.

```yaml
# .github/workflows/docs.yml
- name: mdloom lint
  run: mdloom check . --fail-on-error
```
...
**Goal**: `mdloom compile --watch` reading from mdloom.toml targets.

```bash
# mdloom.toml already has:
# [[compile]]
# source_dir = "src/docs"
# output_dir = "docs"
...
**Covers**: watch mode, multi-target, [[compile]] in mdloom.toml
...
**Goal**: `mdloom check` catches broken references in .source.md files early.

```bash
mdloom check src/guides/          # md_broken_uri errors if any md:// is missing
mdloom compile --check src/guides/ # validates directives without writing
```
...
## US-21 — Library author documenting mdloom-math standalone

**Who**: A Rust developer building a CLI tool that needs terminal math rendering.
**Goal**: Use mdloom-math crate directly for LaTeX → ASCII output.

`src/user-scenarios/21-mdloom-math-demo/main.rs`:
...
**Goal**: `mdloom compile` and `mdloom compile --watch` build both targets automatically.
...
**Goal**: Render `mdloom:blockquote` with a left-bar margin and attribution line.
...
**Goal**: Render a horizontal bar chart directly from the data with `mdloom:chart`.
...
**Goal**: `mdloom:xref` resolves heading text at compile time; when the heading is renamed, one recompile updates all references.
...
**Who**: An ops engineer running `mdloom status` to confirm the corpus is fully compiled and error-free before a docs release.
...
**Covers**: `mdloom status`, `.mdloom/last-check.json`
...
**Goal**: `mdloom check --unused` identifies figures that no `.source.md` references.

```bash
mdloom check . --unused
```

**Covers**: `mdloom check --unused`, `md_unused_figure`
...
**Goal**: `mdloom depends` lists every file to update before making the rename.

```bash
mdloom depends md://api.md#authentication
```

**Covers**: `mdloom depends`, reverse dependency lookup
...
**Goal**: `mdloom spec-generate --ai` calls `claude -p` with the figure content and returns a `[[davinci]]` block.
...
**Covers**: `[ai]` config block, `mdloom spec-generate --ai`, configurable CLI

---

## US-42 — Developer protecting a figure with an inline pin declaration

**Who**: A developer who wants to declare a figure's expected DaVinci pin in the source document, not just in mdloom.toml.
**Goal**: `mdloom:include pin=id` emits COMPILE-007 warning when no matching `[[davinci]]` entry exists.
...
**Who**: A CI engineer running `mdloom check` on a 50-slide deck corpus.
**Goal**: `--deduplicate` collapses `42x SLIDE-001 in docs/slides/*.md` instead of 42 individual lines.

```bash
mdloom check . --deduplicate
```

**Covers**: `mdloom check --deduplicate`, `format_deduplicated`
...
**Goal**: `mdloom check` emits `md_broken_heading` for any `.source.md` that references `md://guide.md#setup`.
...
**Goal**: `mdloom:toc section="Authentication"` lists only headings nested under that section.
...
**Who**: A developer who runs local models and wants `mdloom spec-generate --ai` to use ollama instead of claude.
...
```bash
mdloom spec-generate "md://figures/arch.md:figure:0" --ai
```
...
## US-49 — Writer using mdloom:xref with note format

**Who**: A technical writer who wants "See also" callouts in a prose document.
**Goal**: `mdloom:xref format=note` renders `> **See also:** [Heading](link)`.
...
mdloom compile src/guide.source.md

# Second compile (figure unchanged): 5 cache hits → 0 mdpath calls
mdloom compile src/guide.source.md
```

**Covers**: Tier 2 resolve cache, `resolve_uri_cached`, `.mdloom/cache/resolve/`

---

## Results

Run: `mdloom compile src/user-scenarios/ --output-dir docs/user-scenarios/`

| Scenario | Status | Notes |
|----------|--------|-------|
| US-01 | ✓ check | `mdloom check` catches errors — no source file needed |
...
| US-05 | ✓ CLI | `mdloom spec-generate` — generates DaVinci TOML block |
| US-06 | ✓ CLI | `mdloom fix --min-confidence high` — fix pipeline works |
| US-07 | note | mdloom-canvas is a Rust library — see `src/user-scenarios/26-canvas-tui/main.rs` |
| US-08 | ✓ compiles | Model comparison — mdloom:row from data/models.md |
| US-09 | ✓ compiles | Dependencies — dirtree + bullet lists |
| US-10 | ✓ compiles | Calculus deck — 6 slides, inline + display math |
| US-11 | ✓ CLI | CI integration — `mdloom check . --fail-on-error` works |
| US-12 | ✓ compiles | Blog post with ASCII art — figures preserved intact |
| US-13 | ✓ CLI | `mdloom compile --watch` — rebuilds on save |
| US-14 | ✓ compiles | ML taxonomy — bullet list hierarchy renders cleanly |
| US-15 | ✓ compiles | Rulebook — mdloom:ol numbered lists, callouts, mdloom:right |
| US-16 | ✓ check | Table schemas in mdloom.toml — enforced at lint time |
...
| US-20 | ✓ check | Source link checking — `mdloom check src/` catches broken md:// |
| US-21 | note | mdloom-math is a Rust library — see `src/user-scenarios/27-mdloom-math-binary/main.rs` |
| US-22 | ✓ compiles | Status board — 6-region dashboard, no panic |
| US-23 | ✓ compiles | ADR with TOC — `mdloom:toc` generates numbered outline |
...
| US-26 | ✓ library | mdloom-canvas in ratatui TUI — `src/user-scenarios/26-canvas-tui/main.rs` |
| US-27 | ✓ library | mdloom-math binary — `src/user-scenarios/27-mdloom-math-binary/main.rs` |
| US-28 | ✓ compiles | Large corpus scan — 2,703-file baseline check, 0 errors |
| US-29 | ✓ CLI | Fix pipeline on 47 errors — `mdloom fix --min-confidence high` |
| US-30 | ✓ compiles | CI `--delete-on-error` workflow — stale output cleaned on failure |
| US-31 | ✓ compiles | mdloom:blockquote — bar margin + attribution renders correctly |
| US-32 | ✓ compiles | mdloom:chart bar charts from inline data — two charts |
| US-33 | ✓ compiles | mdloom:xref inline + note format — self-referential headings resolve |
| US-34 | ✓ compiles | mdloom:reveal — 4-slide deck, 2 SLIDE-001 warnings (expected in demo) |
| US-35 | ✓ compiles | Slide footer + layout=agenda — footer on all slides, agenda auto-generated |
| US-36 | ✓ CLI | mdloom status — source/compiled/stale counts, last compile time |
| US-37 | ✓ CLI | mdloom check --unused — md_unused_figure for orphaned figures |
| US-38 | ✓ CLI | mdloom depends — lists all references before renaming |
...
| US-41 | note | mdloom spec-generate --ai — requires claude or other CLI on PATH |
| US-42 | ✓ compiles | mdloom:include pin= — document compiles, COMPILE-007 path explained |
| US-43 | ✓ CLI | mdloom check --deduplicate — collapses repeated warnings |
| US-44 | ✓ CLI | mdloom check catches md_broken_heading on renamed heading |
| US-45 | ✓ compiles | mdloom:toc section= — scoped TOC for Authentication section only |
...
| US-49 | ✓ compiles | mdloom:xref format=note and format=callout both render |
...
2. **Inline body trees** — `kind=org/taxonomy/dependency` with inline body content was
   reverted by `git checkout src/compile.rs`. These scenarios used `mdloom:bullets` as workaround.
...
## US-26 — mdloom-canvas in a ratatui TUI

**Who**: A Rust developer building a terminal monitoring app.
**Goal**: Use mdloom-canvas as the layout primitive — paste regions at exact positions — then hand the rendered string to ratatui for display.
...
`src/user-scenarios/27-mdloom-math-binary/main.rs`
...
## US-29 — mdloom fix pipeline on 47 errors
...
`src/user-scenarios/29-fix-pipeline/mdloom.toml` — zero-tolerance config for checking.

```bash
mdloom check src/user-scenarios/29-fix-pipeline/ --errors-only
mdloom fix   src/user-scenarios/29-fix-pipeline/ --min-confidence high --dry-run
mdloom fix   src/user-scenarios/29-fix-pipeline/ --min-confidence high
mdloom check src/user-scenarios/29-fix-pipeline/ --errors-only
```
...
## US-30 — mdloom compile --delete-on-error CI workflow

**Who**: A CI engineer who wants docs to never deploy with stale compiled output.
**Goal**: `mdloom compile --delete-on-error` removes old output when a source fails to compile, so the deploy step always sees either fresh or absent output — never stale.
