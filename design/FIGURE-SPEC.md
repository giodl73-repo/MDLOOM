...
`<!-- mdloom:figure -->` HTML comment markers:
...
mdloom indexes it. A figure file may contain multiple figures.
...
## `mdloom:figure import=` — image to ASCII

Convert any raster image (PNG, JPG, GIF, BMP) or vector (SVG) to ASCII art.

```bash
mdloom figure import logos/EDM.png --id edm-logo --width 40 --height 20
mdloom figure import logos/EDM.png --id edm-logo --dither block --edge
mdloom figure import photos/McDavid.jpg --id mcdavid-portrait --width 60 --dither braille
mdloom figure import icons/bear.png --id bear --shape octagon --label "STOP" --width 20
```
...
**Content locking:** When a remote URL is fetched, mdloom writes a `.mdloom-fetch-lock.json` alongside the output file recording `{url, fetch_time, sha256_of_response}`. On subsequent compiles, the lock file is checked — if the SHA-256 still matches, the cache hits. If the remote content changes, the cache misses and the figure regenerates. Pinned figures (`mdloom pin`) that include a remote source must have their lock file committed.

**`--output-file` format:** `--output-file` writes the **full markdown wrapper** including `<!-- mdloom:figure id="..." -->` marker, fenced code block, and `<!-- /mdloom:figure -->` closer — the same format as figure files. The raw ASCII content is always wrapped. Use `--output-file -` to write raw ASCII to stdout without the wrapper.
...
mdloom figure import animals/bear.png \
    --shape octagon \
    --label "STOP" \
    --label-pos bottom \
    --width 20 \
...
mdloom figure generate --kind logo-badge --text "EDM" --subtitle "OILERS" \
    --shape shield --width 20 --id edm-badge

# Generate an animal in a shape
mdloom figure generate --kind animal --name bear --shape octagon --label "STOP" \
    --width 20 --id bear-stop

# Generate a geometric shape
mdloom figure generate --kind shape --name star --size 5 --id large-star
```
...
## `mdloom figures .` — figure catalog

List all figures in scope with their metadata:

```bash
mdloom figures .
mdloom figures figures/nhl/
mdloom figures --kind logo
```
...
The `kind` attribute on `<!-- mdloom:figure -->` classifies the figure for
...
Generated figures can be pinned with `mdloom pin` or `mdloom spec-generate`:

```bash
# Generate and immediately pin
mdloom figure import logos/EDM.png --id edm-logo --width 40 | \
    mdloom pin md://figures/edm-logo.md#edm-logo:0 --id edm-logo --protection error

# Or: generate spec, review, then pin
mdloom spec-generate md://figures/edm-logo.md#edm-logo:0 --id edm-logo
```
...
In `.source.md` files, use `mdloom:include kind=figure` to embed a named figure:
...
Use `mdloom:include kind=figure` to embed a named figure. This is the preferred form — it triggers `FIGURE-005` (no DaVinci pin warning) when the `kind=figure` attribute is present, while plain `mdloom:include` without `kind=` does not. Authors using `mdloom:include` without `kind=figure` will not receive the pin reminder.

---

## Integration with mdloom layout
...
mdloom figure generate --kind logo-badge \
...
Each team gets a `figures/nhl/{abbrev}-logo.md` with a `mdloom:figure id="{abbrev}-logo"` block.

**Filename sanitization:** The `--text-field` value (e.g., `abbrev`) is used as the output filename stem. mdloom sanitizes it: lowercase, spaces → hyphens, non-alphanumeric (except `-_`) → stripped. `EDM` → `edm-logo.md`, `St. Louis` → `st-louis-logo.md`. The sanitized stem is also used as the figure `id`. Authors should verify sanitized names are unique across the team table.
...
```bash
mdloom figure import <image>          # convert image to ASCII figure
mdloom figure generate --kind <kind>  # generate from template
mdloom figures [path]                 # list/catalog figures in scope
mdloom figure preview <uri>           # show figure in terminal
```
...
| `FIGURE-005` | warning | Figure has no DaVinci pin — use `mdloom pin` to protect it |
...
- [Compile Spec](./compile-spec.md) — mdloom:figure directive, DaVinci validation
